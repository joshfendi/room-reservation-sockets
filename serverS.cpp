...
#define MYPORT "41966"     // need to bind its own port number
...
{
    // read file
    std::vector<std::pair<std::string, int> > data = readFile("single.txt");
    ...
    while (true)
    {
        ...
        if (AorR == 'A')
        {
            std::cout << "The Server S received an availability request from the main server.\n";
            std::string availabilityCode;
            searchAvailability(receivedRoom, availabilityCode, data);
            sendToMainServer(availabilityCode);

            std::cout << "The Server S finished sending the response to the main server.\n";
        }
        else if (AorR == 'R')
        {
            std::cout << "The Server S received an reservation request from the main server.\n";
            ...
            {
                std::cout << "The Server S finished sending the response and the updated room status to the main server.\n";
            }
